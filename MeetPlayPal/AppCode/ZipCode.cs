...
namespace MeetPlayPal
{
    public class City : ConnManager
    {
        ...
        public bool SetCommandCity(ref SqlCommand CmdSent)
        {
            SqlCommand Cmd = new SqlCommand("City_Sp");
        ...
        public bool CreateCity(ref double NewMasterID, SqlTransaction TrTransaction)
        {
            SqlCommand CmdExecute = new SqlCommand();
            if (SetCommandCity(ref CmdExecute))
            {
                try
                {
